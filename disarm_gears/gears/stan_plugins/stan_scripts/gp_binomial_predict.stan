// Binomial process with an underlying GP
// --------------------------------------
...
data {
  ...
  int<lower=1> n_trials_data[n_data];
  int<lower=1> n_trials_pred[n_pred];
...
model {
  ...
  eta ~ normal(0, 1);
  for (i in 1:n_data) y_data[i] ~  binomial(n_trials_data[i], phi_data[i]);
...
generated quantities {
  ...
  for (i in 1:n_pred) y_pred[i] = binomial_rng(n_trials_pred[i], phi_pred[i]);
